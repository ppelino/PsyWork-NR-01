...
def seed():
    ...
    if not db.query(Company).first():
        comp = Company(name="DEMO LTDA", cnpj="00.000.000/0000-00", plan="demo", response_limit=10000)
        db.add(comp); db.commit()
        user = User(email="admin@demo.com",pwd_hash = pbkdf2_sha256.hash("admin123")
 , role="admin", company_id=comp.id)
        db.add(user)
        dataset = json.loads(open(os.path.join(BASE_DIR,'questions.json'),'r',encoding='utf-8').read())
        for q in dataset:
...
class CompanyIn(BaseModel):
    name: str; cnpj: str | None = None; plan: str | None = "demo"; response_limit: int | None = 10000

class CampaignIn(BaseModel):
    title: str; start_at: datetime | None = None; end_at: datetime | None = None; meta: dict | None = {}

class PublicResponseIn(BaseModel):
    token: str; sector: str | None = None; ghe: str | None = None; ges: str | None = None; environment: str | None = None; answers: dict; notes: str | None = ""
...
def update_company(data: CompanyIn, user=Depends(auth_user), db: Session = Depends(get_db)):
    ...
    comp.name = data.name; comp.cnpj = data.cnpj; comp.plan = data.plan or comp.plan; comp.response_limit = data.response_limit or comp.response_limit
...
def create_campaign(inp: CampaignIn, user=Depends(auth_user), db: Session = Depends(get_db)):
    ...
    camp = Campaign(company_id=user.company_id,title=inp.title, token=token, start_at=inp.start_at or datetime.utcnow(), end_at=inp.end_at, meta=json.dumps(inp.meta or {}))
...
def list_campaigns(user=Depends(auth_user), db: Session = Depends(get_db)):
    camps = db.query(Campaign).filter_by(company_id=user.company_id).order_by(Campaign.id.desc()).all()
    return [{"id":c.id,"title":c.title,"token":c.token,"active":c.active,"start_at":c.start_at,"end_at":c.end_at} for c in camps]
...
def public_respond(data: PublicResponseIn, db: Session = Depends(get_db)):
    ...
    if not camp: raise HTTPException(404,"Campanha não encontrada/ativa")
    comp = db.query(Company).filter_by(id=camp.company_id).first()
    total = db.query(Response).join(Campaign, Campaign.id==Response.campaign_id).filter(Campaign.company_id==comp.id).count()
    if total >= (comp.response_limit or 10000):
        raise HTTPException(403,"Limite de respostas atingido para o plano atual")
    r = Response(campaign_id=camp.id, sector=data.sector, ghe=data.ghe, ges=data.ges, environment=data.environment, answers=json.dumps(data.answers), notes=data.notes or "")
...
def summary(campaign_id:int, user=Depends(auth_user), db: Session = Depends(get_db)):
    ...
    if not camp: raise HTTPException(404,"Campanha não encontrada")
    qs = {q.id:(q.dimension,q.text) for q in db.query(Question).all()}
    items = db.query(Response).filter_by(campaign_id=camp.id).all()
    dim_scores = {}; n=0; rows=[]
    for it in items:
        ans=json.loads(it.answers); n+=1
        for qid,score in ans.items():
            dim=qs[int(qid)][0]; dim_scores.setdefault(dim,[]).append(float(score))
        rows.append({"created_at":it.created_at.isoformat(), "sector":it.sector, "ghe":it.ghe, "ges":it.ges, "environment":it.environment, **{f"Q{qid}":score for qid,score in ans.items()}})
    avg = {d: (sum(v)/len(v) if v else 0) for d,v in dim_scores.items()}
    actions = []
    for d,score in avg.items():
        if score<2.5:
            actions.append({"dimension":d,"priority":"Alta","suggestion":f"Implementar medidas imediatas para {d.lower()} (treinamento, revisão de processos, reforço de recursos, mediação de conflitos)."})
        elif score<3.5:
            actions.append({"dimension":d,"priority":"Média","suggestion":f"Plano de melhoria para {d.lower()} com responsáveis e prazos; comunicar metas e acompanhar trimestralmente."})
        else:
            actions.append({"dimension":d,"priority":"Manter","suggestion":f"Manter boas práticas em {d.lower()} e monitorar com reavaliações semestrais."})
    return {"count": n, "average": avg, "actions": actions, "rows": rows, "campaign":{"id":camp.id,"title":camp.title,"token":camp.token}}
...
def export_csv(campaign_id:int, user=Depends(auth_user), db: Session = Depends(get_db)):
    ...
    items = db.query(Response).filter_by(campaign_id=camp.id).all()
    output = io.StringIO()
    if not items: return JSONResponse({"csv": ""})
